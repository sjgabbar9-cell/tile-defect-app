...
if st.session_state.page == "dashboard":
    ...
    if os.path.exists(CSV_PATH):
        with open(CSV_PATH, "rb") as f:
            st.download_button("⬇ Download Defect History", f, "defect_history.csv")
...
# ---------------- DEFECT ENTRY ----------------
    ...
    for i, defect in enumerate(DEFECTS[dept]):
        with cols[i % 3]:
            ...
            c2.markdown(str(st.session_state.defects[dept][defect]))
...
# ---------------- SUMMARY ----------------
    ...
    defective_tiles = st.number_input("Number of Defect Tiles", min_value=0)
    total_tiles = st.number_input("Total Tiles in Batch", min_value=1)

    if st.button("💾 Save"):
        save_to_csv(st.session_state.batch, st.session_state.defects,
                    defective_tiles, total_tiles)
